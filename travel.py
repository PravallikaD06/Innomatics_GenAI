...
def get_travel_recommendations(source, destination, travel_date):
    """
    Fetches travel recommendations using Google GenAI and returns structured JSON data.
    """
    model = genai.GenerativeModel("gemini-1.5-pro")
    
    query = f"""
    You are a travel assistant. Provide travel options from {source} to {destination} for {travel_date}.
